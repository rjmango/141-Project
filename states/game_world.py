...
class Game_World(State):
    def __init__(self, game):
        ...
        self.rating = [[[0, 0, 40], [0, 0, 50], [0, 1, 00]],
                       [[0, 0, 50], [0, 1, 00], [0, 1, 10]] ,
                       [[0, 1, 00], [0, 1, 10] , [0, 1, 20]] ]
...
class End(pygame.sprite.Sprite):
    ...
    def collidePlayer(self):
        hits = pygame.sprite.spritecollide(self, self.world.all_sprites, False)
        if hits:
            if self.world.current_food == self.world.food_count and hits[0].rect.topleft == self.rect.topleft:
